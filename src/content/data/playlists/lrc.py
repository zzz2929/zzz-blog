...
def replace_lrc(input_file, yrc_path, output_path=None):
    ...
    # 构建 yrc 中歌曲名到 (lrc, album) 的映射
    yrc_map = {}
    for song in yrc_data.get('songs', []):
        name = song.get('name')
        if name:
            yrc_map[name] = {
                'lrc': song.get('lrc'),
    ...
    for song in lironghao_data.get('songs', []):
        name = song.get('name')
        if name and name in yrc_map:
            # 替换 lrc（如果存在）
            if 'lrc' in yrc_map[name]:
                song['lrc'] = yrc_map[name]['lrc']
                replaced_lrc += 1
            # 替换 album（如果存在）
            if 'album' in yrc_map[name]:
                song['album'] = yrc_map[name]['album']
                replaced_album += 1
    ...
    print(f"替换完成，共替换 {replaced_lrc} 首歌曲的 lrc 字段。{replaced_album} 首歌曲的 album 字段。")
    print(f"结果已保存至: {output_path}")

if __name__ == "__main__":
    # 使用示例：文件在脚本所在目录
    script_dir = Path(__file__).parent
    input_file = script_dir / "mijinxuanshi.json"
